...
func InstantResponse(repository *SmartContractRepo, g *gin.Context, request model.VerifyContractRequest) {
	...
	if contract.ContractHash != "" {
		...
		if hash == "" {
			...
			util.PublishRedisMessage(ctx, redisClient, request.ContractAddress, config.REDIS_CHANNEL, "", false, model.ResponseMessage[model.ERROR_GET_HASH])
			return
		}
		...
		if err := model.GetSmartContractByHash(repository.Db, &exactContract, contractHash, model.EXACT_MATCH); err == nil {
			...
			util.PublishRedisMessage(ctx, redisClient, request.ContractAddress, config.REDIS_CHANNEL, dir, true, model.ResponseMessage[model.SUCCESSFUL])
			return
		}
	...
	if verify {
		...
		if err != nil {
			...
			util.PublishRedisMessage(ctx, redisClient, request.ContractAddress, config.REDIS_CHANNEL, "", false, model.ResponseMessage[model.READ_SCHEMA_ERROR])
			return
		}
		...
		for _, file := range files {
			...
			if err != nil {
				...
				util.PublishRedisMessage(ctx, redisClient, request.ContractAddress, config.REDIS_CHANNEL, "", false, model.ResponseMessage[model.READ_SCHEMA_FILE_ERROR])
				return
			}
		...
		util.PublishRedisMessage(ctx, redisClient, request.ContractAddress, config.REDIS_CHANNEL, dir, false, model.ResponseMessage[model.FAILED])
	}
	redisClient.Close()
